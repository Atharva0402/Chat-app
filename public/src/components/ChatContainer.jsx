...
export default function ChatContainer({currentChat,currentUser,socket}) {
  const svgDataUri = `data:image/svg+xml,${encodeURIComponent(currentChat?.avatarImage)}`;
  
  const [messages,setMessages] = useState([]);
  const [arrivalMessages,setArrivalMessages] = useState(null);
  const scrollRef = useRef(null);
  // !thsi the original code 
  // useEffect(async()=>{
  ...
  const  handleSendMsg = async (msg) =>{
    ...
    //  update msgs state by using  the new object of message 
    ...
    // ! original code push isn't a function 
    const msgs = [...messages];
            ...
            messages.map((message)=>{
              return(
                <div ref={scrollRef} key={uuidv4()}>
                   <div className={`message ${message.fromSelf ? "sended": "receieved "}`}>
